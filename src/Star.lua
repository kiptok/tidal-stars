...
function Star:make()
	for k, point in pairs(self.points) do
		local x = round(math.random(-self.size, self.size) * math.random(-self.size, self.size))
		local y = round(math.random(-self.size, self.size) * math.random(-self.size, self.size))
		local c = {} -- 
		for i = 1, 3 do
